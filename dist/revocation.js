...
export const DEFAULT_REVOCATIONS_URL = 'https://tunnelmind.ai/.well-known/receipt-revocations.json';
let _cache = null;
export async function fetchRevocations(opts = {}) {
    if (opts.feed)
        return opts.feed;
    ...
    const r = await f(url);
    if (!r.ok)
        throw new Error(`fetchRevocations: ${url} returned ${r.status}`);
...
export function isKeyRevoked(feed, keyId) {
...
export function isReceiptRevoked(feed, receiptId) {
